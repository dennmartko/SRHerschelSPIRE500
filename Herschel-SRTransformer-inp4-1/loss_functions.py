...
def kernel_loss(gen_output, Y, true_kernel):
    ...
    for i in tf.range(Y.shape[0]):
        ...
        if tf.shape(peaks)[0] != 0 and tf.shape(peaks)[0] is not None:
            ...
            for j in tf.range(tf.shape(peaks)[0]):
...
@tf.function()
def non_adversarial_loss(gen_output, target, w_lh, w_lk, true_kernel, use_lk):
