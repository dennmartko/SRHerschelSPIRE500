...
def confusion_score(Target_catalog, Reconstructed_catalog, confusion_df, matching_args):
...
def fill_pixcatalog(img_batch, cat, ImageIDList, instr_noise):
    ...
    # _, __, std_noise = sigma_clipped_stats(img_batch, sigma=3, maxiters=50)
    # bins = np.linspace(-30*std_noise, 30*std_noise, 500)
    # fig, ax = plt.subplots()
    # pixel_vals = img_batch[0].flatten()
    # hist, bin_edges, _ = ax.hist(pixel_vals[(pixel_vals >= -30*std_noise) & (pixel_vals <= 30*std_noise)], bins=bins)
    # Add a vertical line at the mean pixel value
    # ax.axvline(0, color='red', linestyle='--', label='Mean = {:.2f}'.format(0))
    ...
    # Display the plot
    # plt.show()
    # Initialise the detection method
