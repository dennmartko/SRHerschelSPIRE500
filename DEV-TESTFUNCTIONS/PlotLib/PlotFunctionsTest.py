...
def confusion_plot(flux_bins, confusion_df_list, label_list, save):
    ...
    axs[0].fill_between(np.array([np.min(flux)-0.1, sigma_instr]), 1.1, color='#B7FFA3', alpha=0.3)
    axs[1].fill_between(np.array([np.min(flux)-0.1, sigma_instr]), 1.1, color='#B7FFA3', alpha=0.3)
    ...
    # Save the plot
    fig.savefig(save, dpi=350, edgecolor='white', facecolor='white', bbox_inches='tight')
...
def FluxMatch_Distribution_plot(blind_matches_catalog, rnd_its, save, *catalogs, **matching_args):
    ...
    for i in range(3):
        ...
        axs[i].hlines(y=2., xmin=xlim[0], xmax=xlim[1], linestyle="dashed", color='red', linewidth=1)
        axs[i].vlines(x=2., ymin=ylim[0], ymax=ylim[1], linestyle="dashed", color='red', linewidth=1, label="Detection Threshold")
        ...
        if i == 0 or i == 2:
            ...
            H_ = H_blind if i == 0 else H
            for row_idx, row in enumerate(H_.T): # y
...
def Plot_InputImages(X, save):
    if X.shape[0] == 3:
        ...
        labels = [r"Spitzer MIPS 24 $\mu m$", r"Herschel SPIRE 250 $\mu m$", r"Herschel SPIRE 350 $\mu m$", r"Herschel SPIRE 500 $\mu m$"]

    fig, axs = plt.subplots(1, X.shape[0], figsize=(16, 4.5), sharey=True, sharex=True)
    fig.subplots_adjust(wspace=0.08)
    for i in range(X.shape[0]):
        if X.shape[0] == 4 and i == 0:
            norm = PowerNorm(gamma=0.5, vmin=0, vmax=1)
            im = axs[i].imshow(X[i]*1000, origin='lower', cmap='viridis', norm=norm)
        else:
...
def plot_super_resolved_image(Y, title, cat, ImageID, save, sources=None):
    ...
    if sources is not None:
        ...
        # if r_brightest[rmin_brightest_idx] > 8 and r_median_brightest[rmin_median_brightest_idx] > 8:
    ...
    # The highlighted regions should be away from the border of the image
    if sources is None:
        selected_sources = []
        cat_img_brightest = cat_img.sort_values(by='peak', ascending=False)
        for i in range(cat_img_brightest.shape[0]):
            source = cat_img_brightest.iloc[i]
            x0 = compute_x0(source["xpix"])
            y0 = compute_y0(source["ypix"])
            if y0 > 50 and x0 > 20 and (y0 + WINDOW_SIZE) < Y.shape[1]-50 and (x0 + WINDOW_SIZE) < Y.shape[0]-50:
                if len(selected_sources) == 0:
                    selected_sources.append(source)
                else:
                    # Remove this if statement for more dynamic highlights. Fixed for Thesis.
                    r = np.sqrt((source["xpix"] - selected_sources[-1]["xpix"])**2 + (source["ypix"] - selected_sources[-1]["ypix"])**2)
                    if r>WINDOW_SIZE:
                        selected_sources.append(source)
            if len(selected_sources) == 2:
                brightest = selected_sources[0]
                median_brightest = selected_sources[1]
                break

            if i == cat_img_brightest.shape[0] - 1:
                return (None, None)
    else:
        brightest = sources[0]
    ...
    # Main plot
    norm = PowerNorm(gamma=.7, vmin=0, vmax=30)
    im1 = axs[0].imshow(Y*1000, origin='lower', cmap='viridis', norm=norm, aspect='equal')
    ...
    axs[0].tick_params(axis='x', which='both', labelsize=10)
    axs[0].tick_params(axis='y', which='both', labelsize=10)

    # Highlighted plots
    pos = axs[0].get_position()
    for i in range(2):
        axs[i+1].imshow(Y*1000, origin='lower', cmap='viridis', norm=norm, aspect='equal')
        ...
        axs[i+1].set_xticks(np.arange(np.round(xlim[0]), np.round(xlim[1])+20, 20))
        axs[i+1].set_yticks(np.arange(np.round(ylim[0]), np.round(ylim[1])+20, 20))
        if i == 0:
            axs[i+1].set_position([pos.x0, pos2.y0, pos2.width, pos2.height])
            axs[i+1].scatter(brightest['xpix'], brightest['ypix'], marker="x", s=30, color='red', linewidths=0.6)
            if sources is not None:
                axs[i+1].scatter(gen_brightest['xpix'], gen_brightest['ypix'], marker="x", s=30, color='blue', linewidths=0.6)

        else:
            axs[i+1].set_position([(pos.x0 + pos.width) - pos2.width, pos2.y0, pos2.width, pos2.height])
            axs[i+1].scatter(median_brightest['xpix'], median_brightest['ypix'], marker="x", s=30, color='red', linewidths=0.6)
            if sources is not None:
                axs[i+1].scatter(gen_median_brightest['xpix'], gen_median_brightest['ypix'], marker="x", s=30, color='blue', linewidths=0.6)
        ...
        axs[i+1].tick_params(axis='x', which='both', labelsize=10)
        axs[i+1].tick_params(axis='y', which='both', labelsize=10)
...
def source_profile_comparison(Y_list, sources_arr, instr_noise, save):
    ...
    for i in range(len(Y_list)):
        ...
        if profile_median_brightest.shape[0] != 21 or profile_brightest.shape[0] != 21:
            return
