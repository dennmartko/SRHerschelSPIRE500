...
from loss_functions import non_adversarial_loss, non_adversarial_loss_valid, gaussian2D
...
class SRTrainerGPU:
    ...
    def BuildModel(self):
        if self.first_run:
            ...
            self.generator = tf.keras.models.load_model(os.path.join(self.model_path, 'checkpoint_Model'))
            if self.verbose == True:
                self.printlog(f"{datetime.datetime.now()} - Previous Checkpoint Model Loaded!")
    ...
    def evaluate_metrics(self):
        ...
        for batch_idx in range(its):
            ...
            Lh_batch, Lk_batch = tf.numpy_function(non_adversarial_loss_valid, [gen_valid, Y, self.true_kernel], Tout=[tf.float32, tf.float32])

            Lh_valid += Lh_batch/its
            Lk_valid += Lk_batch/its
        ...
        if len(reconstructed_catalog["peak"]) == 0:
            return 0, 0, np.nan, np.nan, np.nan, PSNR, Lh_valid, Lk_valid
        ...
        if thresh_bool and C != 0:
            ...
            mape = np.nan
            C, R = np.nan, np.nan
        return C, R, d_a, d_b, mape, PSNR, Lh_valid, Lk_valid
    ...
    def Train(self):
        ...
        self.use_lk = config_params['use_lk'].strip() == "True"

        # Parse learning rate parameters
        lr_params = [float(i.strip()) for i in config_params['polynomial_decay'].split(",")]
        use_polynomial_decay = config_params['use_polynomial_decay'].strip().lower() == "true"

        # Initialise the optimizer and learning rate function
        if use_polynomial_decay and self.first_run:
            lr_fn = tf.optimizers.schedules.PolynomialDecay(lr_params[0], lr_params[1] * self.train_arr_X.shape[0]//self.BATCH_SIZE, lr_params[2], lr_params[3])
        else:
            lr_fn = 1e-4

        generator_optimizer = tf.keras.optimizers.Adam(lr_fn, beta_1=0.9)

        @tf.function()
        def train_step_G(X, Y, w_lh, w_lk, true_kernel): #discriminator, discriminator_optimizer
            # Update generator with non/-adversarial loss
            with tf.GradientTape() as gen_tape:
                gen_output = self.generator(X, training=True)
                gen_loss = non_adversarial_loss(gen_output, Y, w_lh, w_lk, true_kernel, self.use_lk)
        ...
        num_vis = 9
        EPOCH_THRESHOLD_MODELSAVING = 100 # Must be divisible by 10
        EPOCH_DECAY_PARAM = 350
        n_stopping = 0
        w_lk_start = 0.
        w_lk_end = .1
        kernel_size = 21
        ...
        self.true_kernel = gaussian2D(xx, yy, center_float, center_float, sigma, sigma)
        ...
        if not self.first_run:
            ...
            valid_metrics = {"PSNR":[], "d_a":[], "d_b":[], "mape":[], "Cglobal": [], "Rglobal": [], "epochs":[], "Lh":[], "Lk":[]}
        ...
        for epoch in tqdm(self.n_epochs_arr, desc="Training GAN..."):
            ...
            for i in range(its):
                ...
                w_lk = (epoch / EPOCH_DECAY_PARAM) * (w_lk_end - w_lk_start) + w_lk_start if epoch <= EPOCH_DECAY_PARAM else w_lk_end
                w_lk = tf.cast(w_lk, tf.float32)
                w_lh = tf.cast(1. - w_lk, tf.float32)

                gen_loss = train_step_G(X, Y, w_lh, w_lk, self.true_kernel)
            ...
            if epoch >= EPOCH_THRESHOLD_MODELSAVING and epoch % 10 == 0:
                ...
                C, R, d_a, d_b, mape, PSNR, Lh_valid, Lk_valid = self.evaluate_metrics()
                ...
                valid_metrics["Lh"].append(Lh_valid)
                valid_metrics["Lk"].append(Lk_valid)
                ...
                self.generator.save(os.path.join(self.model_path, 'checkpoint_Model'))
    ...
    def TrainingAnalysis(self):
        ...
        plot_line_chart(losshist["epochs"], [losshist["G loss"]], "Epoch Number", r"$\mathcal{L}_G^{non-adv}$", labels=[""], save_path=os.path.join(self.tdir_out_analysis, 'non_adv_G_loss.pdf'), log_scale=True)
